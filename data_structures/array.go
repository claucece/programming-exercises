...
func make(types string, length, capacity int) []int {
	if length < capacity {
		var slice2 types
...
func append(original []int, position int, value int) []int {
...
func delete(original []int, d int, order bool) []int {
	var target [d]int
	if order == true {
		target = append(original[:d], original[d+1:])
	} else {
